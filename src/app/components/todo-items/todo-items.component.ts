...
import { faAdd, faCheckCircle, faEdit } from '@fortawesome/free-solid-svg-icons';
...
export class TodoItemsComponent implements OnInit {
    ...
    constructor(private activedRoute: ActivatedRoute, private apiService: ApiService, private dialog: MatDialog, private snackBar: MatSnackBar) {}
            ...
            next: (res: any) => {
                if (res) {
                    if (!data) {
                        ...
                        this.apiService
                            .updateTodoItems(this.todoId, this.todoItems[idx].id, {
                                item: res,
                            })
                            .subscribe({
                                next: (res) => {
    ...
    checkedItem(idx = 0) {
        this.apiService
            .updateTodoItems(this.todoId, this.todoItems[idx].id, {
                item: { checked: !this.todoItems[idx].checked },
            })
            .subscribe({
                next: (res) => {
                    this.todoItems[idx] = res;
